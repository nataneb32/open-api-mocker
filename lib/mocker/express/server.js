...
class Server {
	...
	setRoute(app, path) {
		...
		app[expressHttpMethod](uris, handleRequest(path, this.sendResponse, this.validationErrorFormatter || Server._defaultValidationErrorFormatter));
	...
	static _defaultValidationErrorFormatter(errors) {
		return { errors: errors.map(error => `Invalid request at ${error.dataPath.replace(/^\./, '')}: ${error.message}`) };
	}
